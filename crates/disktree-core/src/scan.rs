...
pub struct ScanOptions {
    ...
    /// Stay on the root's volume: skip other disks, device nodes, network
    /// shares and automount points, but keep both halves of the boot
    /// volume group. See [`crate::volumes::excluded_mounts`]. On by
    /// default: a disk tool measures a disk, and its free-space meter only
    /// means anything for one volume.
...
pub struct ScanSnapshot {
    ...
    pub bytes: u64,
    pub errors: u64,
    /// Of `errors`, how many were refused by the OS's privacy protection
    /// rather than by permission bits: `EPERM` on `~/Library/Mail`,
    /// `~/.Trash` and the like, which Full Disk Access unlocks.
    pub denied: u64,
    pub finished: bool,
    pub cancelled: bool,
...
/// Whether Full Disk Access would let a scan into `path` when the kernel
/// answered `EPERM`.
///
/// The grant unlocks user data the privacy protection guards: homes
/// (`~/Library/Mail`, `~/.Trash`, other users' homes) and what is on
/// other volumes (their `.Trashes`, Time Machine backups). It does not
/// unlock a *data vault*: `/private/var/protected`, the daemons' caches
/// under `/Library/Caches/com.apple.*` and their like answer `EPERM` to
/// every process however it is entitled, so counting them would keep the
/// notice up after the grant is in place. Both spellings of the data
/// volume count, since the walk may reach either.
fn full_disk_access_unlocks(path: &Path) -> bool {
    let rest = path.strip_prefix("/System/Volumes/Data").unwrap_or(path);
    // `strip_prefix` drops the leading slash, so compare without it;
    // `starts_with` is by component, so `Users` cannot match `UsersX`.
    let rest = rest.strip_prefix("/").unwrap_or(rest);
    rest.starts_with("Users") || rest.starts_with("Volumes")
}
...
impl ScanProgress {
    ...
    fn record_error(&self, path: &Path, error: &io::Error) {
        ...
        // `EPERM` rather than `EACCES`: the permission bits would allow it,
        // the privacy protection (TCC) does not. Counted apart so the app
        // can suggest Full Disk Access instead of a chmod.
        let not_permitted = error.raw_os_error()
            == Some(rustix::io::Errno::PERM.raw_os_error());
        let message = if not_permitted && full_disk_access_unlocks(path) {
            self.denied.fetch_add(1, Ordering::Relaxed);
            format!("{}: not permitted (privacy)", path.display())
        } else if not_permitted {
            format!("{}: not permitted (system data vault)", path.display())
        } else {
...
#[derive(Clone, Debug)]
pub struct Known {
    /// Where it is. Canonicalized when the scan starts, like the root, so
    /// the two compare textually whatever form the caller used.
    pub path: PathBuf,
    pub tree: Arc<Node>,
}
...
struct WalkContext {
    ...
    /// Mount points the walk must not enter, from the mount table, in every
    /// spelling the walk can reach them by. Empty when the table cannot be
    /// read, and only the device backstop applies.
    excluded: OnceLock<FxHashSet<PathBuf>>,
    /// Devices of the root's volume group, the `one_filesystem` backstop
    /// for a mount the table did not list. Unset without `one_filesystem`.
    allowed_devices: OnceLock<FxHashSet<u64>>,
...
impl WalkContext {
    fn new(
        ...
        // Same form as the root the walk builds its paths from; see
        // `scan_blocking`.
        let known = known.map(|known| Known {
            path: known.path.canonicalize().unwrap_or(known.path),
            tree: known.tree,
        });
        Self {
            known,
            ...
            allowed_devices: OnceLock::new(),
            visited_dirs: Mutex::new(FxHashSet::default()),
            root: Mutex::new(None),
        }
    }

    fn on_own_volume(&self, device: u64) -> bool {
    ...
    fn classify(&self, entry: &DirEntry) -> Classified {
        ...
        if file_type.is_dir() {
            ...
            // Checked by path before anything stats the directory, so an
            // automount trigger is never touched. This applies under `-X`
            // too; the list is then only what can never hold user data.
            if self
                .excluded
                .get()
            ...
            if self.options.one_filesystem {
                // Backstop for a mount the table did not list: a directory
                // on a device outside the volume group is another
                // filesystem, however it got there.
                match entry.metadata() {
                    Ok(meta) if !self.on_own_volume(device_of(&meta)) => {
                        return Classified::Skipped;
                    }
                    Err(error) => {
                        self.progress.record_error(&path, &error);
...
fn scan_blocking(root: &Path, context: &Arc<WalkContext>) -> io::Result<Node> {
    // One spelling of the root everywhere: the mount table, the exclusion
    // list and a memoized subtree all compare paths textually, and on macOS
    // `/tmp`, `/var` and `/etc` are symlinks into `/private`, so a walk
    // built from the given form would never match a table built from the
    // canonical one.
    ...
    leave_dataless_files_alone();
    let one_filesystem = context.options.one_filesystem;
    if let Some(excluded) =
        crate::volumes::excluded_mounts_for(&root, one_filesystem)
    {
        let _ = context.excluded.set(excluded.into_iter().collect());
    }
    if one_filesystem {
        // The mount points of the group are known volumes, never automount
        // triggers, so reading their devices is safe.
        let mut devices = FxHashSet::default();
        devices.insert(device_of(&root_meta));
        for point in crate::volumes::volume_group_for(&root) {
            if let Ok(meta) = fs::metadata(&point) {
                devices.insert(device_of(&meta));
            }
        }
        let _ = context.allowed_devices.set(devices);
    }
    ...
    let root_dir = Arc::new(PendingDir::new(root.clone(), None, 0));
...
///
/// On APFS this stays honest for compressed files (compressed blocks) and
/// for evicted iCloud files (zero). The one thing it cannot see is a clone
/// (`cp -c`, Finder's duplicate): each copy reports its full blocks while
/// they share them on disk, so deleting one copy frees nothing. There is no
/// POSIX API for that, and a treemap that counts what a file *would* cost
/// alone is still the useful one.
///
...
/// Tell the kernel not to fetch evicted iCloud and File Provider files when
/// the walk reads their directories: a dataless placeholder is `st_blocks`
/// zero, which is exactly what it costs, and downloading a home directory's
/// worth of them to measure it would be the opposite of the point.
///
/// Process-wide, so it is set once per scan rather than per thread; the
/// constants are `IOPOL_TYPE_VFS_MATERIALIZE_DATALESS_FILES`,
/// `IOPOL_SCOPE_PROCESS` and `IOPOL_MATERIALIZE_DATALESS_FILES_OFF` from
/// `<sys/resource.h>`, which the libc crate does not carry.
#[cfg(target_os = "macos")]
#[allow(unsafe_code, reason = "setiopolicy_np is not in the libc crate")]
fn leave_dataless_files_alone() {
    use std::ffi::c_int;
    ...
    // SAFETY: three plain integers in, one out; the call touches no memory
    // of ours. A failure only means placeholders may be fetched, which is
    // slower, not wrong.
    let _ = unsafe {
        setiopolicy_np(
...
const fn leave_dataless_files_alone() {}
...
mod tests {
    ...
    /// The notice must not outlive the grant: a data vault answers
    /// `EPERM` to everyone, and only what Full Disk Access unlocks counts.
    #[test]
    fn full_disk_access_unlocks_homes_and_volumes_but_not_data_vaults() {
        for path in [
            "/Users/x/Library/Mail",
            "/Users/x/.Trash",
            "/System/Volumes/Data/Users/x/Library/Safari",
            "/Volumes/Backup/.Trashes",
        ] {
            assert!(full_disk_access_unlocks(Path::new(path)), "{path}");
        }
        for path in [
            "/private/var/protected/trustd/private",
            "/Library/Caches/com.apple.aned",
            "/System/Volumes/Data/private/var/db/x",
            "/UsersX/y",
        ] {
            assert!(!full_disk_access_unlocks(Path::new(path)), "{path}");
        }
    ...
    fn whole_disk_smoke() {
        ...
        let skipped = crate::volumes::excluded_mounts_for(&root, true)
            .unwrap_or_default();
