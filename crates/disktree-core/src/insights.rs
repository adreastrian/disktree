...
//!
//! On a Mac the reclaimable findings are what `classify` hatches under
//! `~/Library`: Xcode's `DerivedData`, `DeviceSupport` and simulator
//! `Devices`, and every `Caches` and `Logs`, Homebrew's included. Device
//! backups under `MobileSync` are deliberately not on the list: they are the
//! only copy of a phone, and this list only names what could plausibly go.
...
fn visit(
    ...
    let experiments = scratch && name == "experiments";
...
mod tests {
    ...
    /// The parts of a Mac's ~/Library that fill a disk.
    fn mac_home() -> Node {
        scan(dir(
            "arif",
            vec![dir(
                ...
                vec![
                    dir(
                        "Caches",
                        vec![dir("Homebrew", vec![file("bottle", 6 * GIB, 2)])],
                    ),
                    dir("Logs", vec![file("app.log", GIB, 2)]),
                    dir(
                        "Developer",
                        vec![
                            dir(
                                ...
                                vec![
                                    ...
                                    dir(
                                        "Archives",
                                        vec![file("dSYM", 3 * GIB, 30)],
                                    ),
                                    dir(
                                        "iOS DeviceSupport",
                                        vec![file("Symbols", 5 * GIB, 60)],
                                    ),
                                ],
                            ),
                            dir(
                                ...
                                vec![
                                    dir(
                                        "Devices",
                                        vec![file("data", 7 * GIB, 5)],
                                    ),
                                    dir(
                                        "Caches",
                                        vec![file("dyld", 2 * GIB, 5)],
                                    ),
                                ],
                            ),
                        ],
                    ...
                    dir(
                        ...
                        vec![dir(
                            "MobileSync",
                            vec![dir("Backup", vec![file("b", 8 * GIB, 100)])],
                        )],
                    ),
                ],
            )],
    ...
    #[test]
    fn xcode_and_homebrew_leftovers_are_findings_but_backups_are_not() {
        let root = mac_home();
        let found = worth_a_look(&root, NOW, 10);
        let lines: Vec<(String, &Finding)> = found
            .iter()
            .map(|c| {
                let node = root.resolve(&c.crumbs).expect("resolves");
                (node.name.to_string(), &c.finding)
            })
            .collect();
        assert_eq!(
            lines,
            vec![
                ...
                (
                    "Devices".to_string(),
                    &Finding::Reclaimable(Reclaim::Reinstallable)
                ),
                (
                    "Caches".to_string(),
                    &Finding::Reclaimable(Reclaim::Regenerable)
                ...
                (
                    "Logs".to_string(),
                    &Finding::Reclaimable(Reclaim::Temporary)
                ),
            ]
        );
        // ~/Library/Caches is one line; Homebrew inside it is not another.
        assert_eq!(found[2].bytes, 6 * GIB);
        // Archives hold the only dSYMs; backups hold the only copy of a
        // phone. Neither is a finding, however large.
        assert!(!lines.iter().any(|(name, _)| name == "Archives"
            || name == "Backup"
            || name == "MobileSync"));
    }
