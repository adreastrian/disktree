...
use disktree_core::volumes::{Volume, scannable_volumes};
use gpui_kit::{
    Context, FocusHandle, KeyDownEvent, MouseButton, MouseDownEvent,
    MouseMoveEvent, PathPromptOptions, Pixels, Point, Render, ScrollDelta,
    ScrollWheelEvent, Size, Window, px, size,
};
use rustc_hash::{FxHashMap, FxHashSet};

use crate::controls::Status;
...
/// The trail's volume menu, open: the row the arrow keys are on, as an
/// index into [`Disktree::volumes`].
#[derive(Clone, Debug, PartialEq, Eq)]
...
/// One wheel notch, as a zoom factor.
const ZOOM_NOTCH: f32 = 1.15;

/// A scroll "line" in pixels: what one wheel notch pans by, and how much
/// trackpad travel counts as one notch.
...
pub struct Disktree {
    ...
    /// The Full Disk Access notice was dismissed for this scan.
    privacy_dismissed: bool,
    pub focus: FocusHandle,
    /// A trail crumb's sibling menu, when open.
    pub crumb_menu: Option<CrumbMenu>,
    /// The trail's volume menu, when open.
    pub volume_menu: Option<VolumeMenu>,
    /// The volumes the menu offers, read from the mount table when it
    /// opens so a disk plugged in since is listed, each with its space
    /// when `statfs` answers.
    pub volumes: Vec<Volume>,
    pub volume_space: Vec<Option<SpaceInfo>>,
    ...
    /// scans and where the trail begins. `/` for the boot volume group,
    /// the mount point for any other volume.
...
impl Disktree {
    ...
    /// `g`: the whole disk the scanned root lives on. Widens when the disk
    /// is above the scanned root, and goes to its top when it already is
    /// the root.
    ...
    /// The trail, from the volume: the directories above the scanned root
    /// down to the top of its volume, which widen the scan, then the root
    /// and the path into the tree.
    ///
    /// It stops at the volume because a scan stays on one: `/` above
    /// `/Volumes/Space` would be the boot disk, whose walk leaves `Space`
    /// out, so the tree on screen would be lost rather than widened. The
    /// first crumb's menu is the way to another volume.
    pub fn breadcrumbs(&self) -> Vec<(String, Crumb)> {
        let top = self
            .disk_root
            .as_deref()
            .filter(|disk| self.root_path.starts_with(disk));
        let mut trail: Vec<(String, Crumb)> = self
            .root_path
            .ancestors()
            .skip(1)
            .take_while(|path| top.is_none_or(|top| path.starts_with(top)))
            .collect::<Vec<_>>()
            .into_iter()
            .rev()
    ...
    /// Open the trail's volume menu, on the volume being scanned. The
    /// mount table is read now, not at start, so a disk plugged in since
    /// is offered.
    pub fn open_volume_menu(&mut self, cx: &mut Context<'_, Self>) {
        self.crumb_menu = None;
        self.volumes = scannable_volumes();
        self.volume_space = self
            .volumes
            .iter()
            .map(|volume| space_info(&volume.root).ok())
            .collect();
        let highlighted = self
            .volumes
            .iter()
            .position(|volume| Some(&volume.root) == self.disk_root.as_ref())
            .unwrap_or(0);
        self.volume_menu = Some(VolumeMenu { highlighted });
        cx.notify();
    }

    /// Scan a volume from its top: a fresh scan, since a volume's walk
    /// never reaches another. Going to the one already scanned is only
    /// widening, and to the one already at the root is going to its top.
    pub fn choose_volume(&mut self, index: usize, cx: &mut Context<'_, Self>) {
        self.volume_menu = None;
        let Some(root) = self.volumes.get(index).map(|v| v.root.clone()) else {
            cx.notify();
            return;
        };
        if self.disk_root.as_ref() == Some(&root) {
            self.go_to_disk(cx);
        } else {
    ...
    /// Keys while the volume menu is open, the same as a sibling menu's.
    fn on_volume_menu_key(
        &mut self,
        key: &str,
        cx: &mut Context<'_, Self>,
    ) -> bool {
        let Some(menu) = self.volume_menu.clone() else {
            return false;
        };
        let last = self.volumes.len().saturating_sub(1);
        let mut highlighted = menu.highlighted;
        match key {
            "down" | "j" => highlighted = (highlighted + 1).min(last),
            "up" | "k" => highlighted = highlighted.saturating_sub(1),
            "home" => highlighted = 0,
            "end" => highlighted = last,
            "enter" | "space" | "right" | "l" => {
                self.choose_volume(highlighted, cx);
                return true;
            }
            "escape" | "left" | "h" => {
                self.volume_menu = None;
    ...
    /// Interface zoom, from `⌘ =`, `⌘ -` and `⌘ 0` or the View menu.
    /// Changes the window's `rem`, which every size in this app is
    /// expressed in, so hierarchy and spacing keep their proportions at
    /// every step.
    pub fn zoom_interface(&mut self, step: InterfaceZoom, window: &mut Window) {
        ...
        let index = steps
            ...
            .unwrap_or(2);
        let next = match step {
            InterfaceZoom::In => (index + 1).min(steps.len() - 1),
            InterfaceZoom::Out => index.saturating_sub(1),
            InterfaceZoom::Reset => 2,
        };
        let rem = crate::ui::BASE_REM * steps[next];
        window.set_rem_size(px(rem));
        // The title bar grows with the rem; the traffic lights do not, so
        // they are re-centred on it.
        window.set_traffic_light_position(
            crate::ui::chrome::traffic_light_position(rem),
        );
        self.cache = None;
    }

    /// The window title: the app, and the directory on screen.
    ...
    /// `⌘ O`, and the Finder's Open With or a folder dropped on the Dock
    /// icon: scan `root` instead of what is on screen. Anything that is not
    /// a directory is ignored rather than reported, since the request came
    /// from the platform and not from a field the user could correct.
    pub fn open_folder(&mut self, root: PathBuf, cx: &mut Context<'_, Self>) {
        // Canonical, so a later widening recognises this tree in the
        // wider walk, as the command line does.
    ...
    /// The native folder picker, then [`Self::open_folder`] on its answer.
    pub fn prompt_for_folder(window: &Window, cx: &Context<'_, Self>) {
    ...
    /// `c`, and View \u{25b8} Review Marked: to the review screen, or back
    /// from it. With nothing marked there is nothing to review, and the
    /// notice says which key marks.
    pub fn toggle_review(&mut self, cx: &mut Context<'_, Self>) {
        match self.screen {
            Screen::Review => self.screen = Screen::Explore,
            Screen::Explore if self.marks.is_empty() => {
                self.notice = Some((
            ...
            Screen::Explore => self.screen = Screen::Review,
            Screen::Running | Screen::Done => return,
        }
        cx.notify();
    }

    /// `/`, `s` and `⌘ F`: open the find field. Only the explore screen
    /// has one.
    pub fn open_find(&mut self, cx: &mut Context<'_, Self>) {
        if self.screen == Screen::Explore && !self.confirm_open {
            self.find_open = true;
            cx.notify();
        }
    }
    ...
    /// The scan met the OS's privacy protection: how many entries it was
    /// not permitted to read, or `None` once the notice was dismissed or
    /// when nothing was refused.
    pub fn privacy_denials(&self) -> Option<u64> {
        (!self.privacy_dismissed && self.progress.denied > 0)
            .then_some(self.progress.denied)
    }

    /// Hide the privacy notice until the next scan.
    pub fn dismiss_privacy_notice(&mut self, cx: &mut Context<'_, Self>) {
        self.privacy_dismissed = true;
        cx.notify();
    }
    ...
    fn dispatch_key(
        ...
        // A ⌘ chord is a shortcut or nothing. The keymap claims the bound
        // ones before they get here; one that reaches this far is unbound,
        // and must neither type into the find field nor fire the letter
        // under it.
        ...
        if self.volume_menu.is_some() && self.on_volume_menu_key(key, cx) {
            return;
        }
    ...
    fn on_explore_key(
        ...
        match key {
            "/" | "s" if !control => self.open_find(cx),
            ...
            "c" if !control => self.toggle_review(cx),
    ...
    /// A mouse wheel zooms, notch by notch, toward the pointer. A trackpad
    /// reports exact pixels: two fingers pan the magnified view, and with
    /// ⌘ held they zoom, smoothly, since the same gesture would otherwise
    /// fire a notch per event and fly through the levels. Shift with either
    /// pans, as before.
    pub fn on_scroll_wheel(
        ...
        let (delta, precise) = match event.delta {
            ScrollDelta::Lines(delta) => (
                Point::new(delta.x * LINE_PIXELS, delta.y * LINE_PIXELS),
                false,
            ),
            ScrollDelta::Pixels(delta) => {
                (Point::new(delta.x.as_f32(), delta.y.as_f32()), true)
            }
        };
        let zoom = if precise {
        ...
        if zoom {
            ...
            let factor = if precise {
                // A finger's worth of travel is about a notch; capped so a
                // flick cannot skip the ceiling where zooming descends.
                (delta.y / LINE_PIXELS * ZOOM_NOTCH.ln())
                    .clamp(-ZOOM_NOTCH.ln(), ZOOM_NOTCH.ln())
                    .exp()
            } else if delta.y > 0.0 {
                ZOOM_NOTCH
            } else {
        ...
        let area = self.treemap_size.get();
        // Pixels on screen are pixels of base space divided by the scale,
        // so the content follows the fingers exactly at any magnification.
        self.view.origin_x -= delta.x / self.view.scale;
        self.view.origin_y -= delta.y / self.view.scale;
...
/// A step of interface zoom.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum InterfaceZoom {
    In,
    Out,
    Reset,
}
