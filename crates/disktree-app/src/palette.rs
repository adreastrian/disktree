...
//! from the active theme, so the mosaic sits inside it, light or dark.
...
pub fn category_fill(theme: &Theme, category: Category, depth: u32) -> Hsla {
    ...
    // The light canvas is nearly white, so its fills carry a little more
    // colour and sit a step darker than a pastel would: the dark hatch and
    // the black labels need something to stand against.
    let (s, l) = if dark(theme) {
        (0.26 * chroma, step.mul_add(0.028, 0.215))
    } else {
        (0.34 * chroma, step.mul_add(-0.03, 0.80))
    };
...
pub fn hatch(theme: &Theme) -> Hsla {
    if dark(theme) {
        ...
        // The light label colour is itself translucent, so the hatch asks
        // for more of it to land at the same strength as the dark one.
...
mod tests {
    ...
    fn theme(appearance: ThemeAppearance) -> Theme {
        match appearance {
            ThemeAppearance::Dark => Theme::dark(),
