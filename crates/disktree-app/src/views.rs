...
use gpui_kit::{
    ...
    MouseButton, ParentElement, Rems, SharedString, Stateful,
...
use crate::controls::{
    ButtonVariant, ChoiceItem, IconName, alert_dialog, button, button_group,
    checkbox, dialog_button, dialog_description, dialog_popup, dialog_title,
    icon, keycap, separator, with_tooltip,
};
use crate::menu;
use crate::palette;
use crate::state::{
    ColorMode, Crumb, Disktree, InterfaceZoom, PANEL_REMS, Screen, panel_width,
};
use crate::theme::{ActiveTheme, Theme};
use crate::treemap_view::{self, Mosaic};
use crate::ui::{chrome, icon, radius, size, space, text};
use crate::widgets;
...
pub fn root(
    ...
    let body = match app.screen {
        ...
        Screen::Running => running(app, window, cx),
        Screen::Done => done(app, window, cx),
    };
    ...
    let mut root = div()
        ...
        // The menu bar and its ⌘ chords land here, on whichever window is
        // in front, and answer with the same methods the plain keys use.
        .on_action(cx.listener(|this, _: &menu::ZoomIn, window, cx| {
            this.zoom_interface(InterfaceZoom::In, window);
            cx.notify();
        }))
        .on_action(cx.listener(|this, _: &menu::ZoomOut, window, cx| {
            this.zoom_interface(InterfaceZoom::Out, window);
            cx.notify();
        }))
        .on_action(cx.listener(|this, _: &menu::ZoomReset, window, cx| {
            this.zoom_interface(InterfaceZoom::Reset, window);
            cx.notify();
        }))
        .on_action(cx.listener(|this, _: &menu::Rescan, _, cx| {
            this.start_scan(cx);
        }))
        .on_action(cx.listener(|this, _: &menu::WholeDisk, window, cx| {
            this.go_to_disk(cx);
            window.set_window_title(&this.window_title());
        }))
        .on_action(cx.listener(|this, _: &menu::FocusFind, _, cx| {
            this.open_find(cx);
        }))
        .on_action(cx.listener(|this, _: &menu::ToggleReview, _, cx| {
...
/// The band macOS keeps for the title bar, drawn by the app: the height
/// the traffic lights are centred on, room for them on the left, and the
/// bar's own manners — drag to move the window, double-click for what
/// System Settings says a title bar double-click does. A control placed in
/// it stops the mouse-down (see [`no_drag`]), so a click on one is never
/// the start of a drag and a double-click on one never zooms the window.
fn title_band(theme: &Theme, window: &Window) -> Stateful<Div> {
    // Full screen hides the lights, and their room goes with them.
    let inset: gpui_kit::AbsoluteLength = if window.is_fullscreen() {
        space::LG.into()
    } else {
        chrome::TRAFFIC_LIGHTS.into()
    };
    div()
        .id("title-bar")
...
/// A region of the title bar that is content, not handle: the mouse-down
/// ends here instead of reaching the bar's window-move.
fn no_drag(region: Div) -> Div {
    region.on_mouse_down(MouseButton::Left, |_, _, cx| cx.stop_propagation())
}

/// The app, where you are, and the controls that decide what is measured,
/// in the title bar.
fn top_bar(
    app: &Disktree,
    theme: &Theme,
    ...
    title_band(theme, window)
        .gap(space::LG)
        .child(logo(theme))
        // Where you are is navigation, and it belongs to the whole window.
        .child(no_drag(trail(app, theme, cx)))
        .child(div().flex_1())
        .child(no_drag(view_settings(app, window, cx)))
}
...
fn trail(app: &Disktree, theme: &Theme, cx: &Context<'_, Disktree>) -> Div {
    ...
    for (index, (label, step)) in steps.into_iter().enumerate() {
        ...
        // The first crumb is the volume: its ▾ lists the others, since a
        // scan never crosses into another volume and the trail cannot
        // climb to one.
        if index == 0 {
            crumb = volume_crumb(app, theme, crumb, cx);
        }
        row = row.child(crumb);
    }
    row
...
/// The volume crumb: the crumb itself, then a ▾ that opens the volume
/// menu under it.
fn volume_crumb(
    app: &Disktree,
    theme: &Theme,
    crumb: gpui_kit::AnyElement,
    cx: &Context<'_, Disktree>,
) -> gpui_kit::AnyElement {
    let open = app.volume_menu.is_some();
    let chip = div()
        .flex()
        .flex_row()
        .items_center()
        .when(open, |this| this.bg(theme.hover_fill()))
        .child(crumb)
        .child(
            div()
                .id("volume-menu-chevron")
                .debug_selector(|| "volume-menu-chevron".into())
                .px(space::XS)
                .py(space::XXS)
                ...
                .on_click(cx.listener(|this, _, _, cx| {
                    if this.volume_menu.is_some() {
                        this.volume_menu = None;
                        cx.notify();
                    } else {
                        this.open_volume_menu(cx);
                    }
                }))
                .child("▾"),
        );
            ...
            this.child(
                deferred(
                    anchored()
                        ...
                        .child(volume_menu(app, theme, cx)),
                )
                .with_priority(2),
            )
        })
...
/// The volumes this Mac shows, the one on screen in bold, each with how
/// full it is: choosing another scans it from its top.
fn volume_menu(
    app: &Disktree,
    theme: &Theme,
    cx: &Context<'_, Disktree>,
) -> impl IntoElement {
    let highlighted = app.volume_menu.as_ref().map(|menu| menu.highlighted);
    let mut panel = div()
        .id("volume-menu")
        .debug_selector(|| "volume-menu".into())
        .occlude()
        .flex()
        .flex_col()
        .w(size::SIBLING_MENU)
    ...
    for (index, volume) in app.volumes.iter().enumerate() {
        let current = app.disk_root.as_ref() == Some(&volume.root);
        let space = app.volume_space.get(index).copied().flatten();
        let (fill, figure) = space.map_or((0.0, String::new()), |space| {
            (
                space.used_fraction(),
                format!(
                    "{} of {} free",
        ...
        let accent = palette::highlight(theme);
        panel = panel.child(
            div()
                    ...
                    div()
                        ...
                        .child(
                            div()
                                ...
                                .overflow_hidden()
                                .text_ellipsis()
                                .child(volume.root.display().to_string()),
                        ),
                )
                .child(
                    div()
                        .flex_shrink_0()
                        .w(size::ROW_BAR)
                        .child(widgets::bar(fill, accent, cx)),
                )
                .child(
                    div()
                        .flex_shrink_0()
...
fn view_settings(
    ...
    div()
        .debug_selector(|| "view-settings".into())
        .flex()
        .flex_row()
        .items_center()
        .gap(space::SM)
...
/// protection refused is flagged in the warning colour, since a grant
/// fixes it; what only root can read is counted in the ordinary colour
/// and explained on hover, since nothing the user can grant changes it and
/// every whole-disk scan meets a few hundred of them.
fn scan_totals(app: &Disktree, theme: &Theme) -> Div {
    ...
    let denied = app.progress.denied;
    div()
        .flex()
        .flex_row()
        .items_center()
        ...
        .when(errors > 0, |this| {
            this.child(with_tooltip(
                ...
                "Directories only root can read, such as /private/var/db, \
                 and the system's data vaults. No permission unlocks them; \
                 what is in them is small and is not counted.",
            ))
        })
        .when(denied > 0, |this| {
            this.child(div().text_color(theme.warning).child(format!(
                "· {} not permitted",
                widgets::human_count(denied)
            )))
        })
}
...
fn side_panel(
    ...
    div()
        ...
        .children(privacy_notice(app, theme, cx))
        .child(disk_section(app, theme, cx))
}

/// System Settings, opened on the Full Disk Access list.
const FULL_DISK_ACCESS_URL: &str = "x-apple.systempreferences:\
com.apple.settings.PrivacySecurity.extension?Privacy_AllFiles";

/// The scan was refused somewhere by macOS privacy protection: say how
/// much was not measured, and where the permission lives. Dismissible,
/// since the answer may well be "that is fine".
fn privacy_notice(
    app: &Disktree,
    theme: &Theme,
    cx: &Context<'_, Disktree>,
) -> Option<Div> {
    let denied = app.privacy_denials()?;
    let noun = if denied == 1 {
        "item was"
    } else {
        "items were"
    };
    Some(
        div()
            .debug_selector(|| "privacy-notice".into())
            .flex()
            .flex_col()
            .gap(space::SM)
            .px(space::SM)
            ...
            .text_color(theme.foreground)
            .child(format!(
                "{} {noun} not permitted (macOS privacy). Grant Full Disk \
                 Access to Disktree in System Settings \u{25b8} Privacy & \
                 Security to measure them. Already on? Turn it off and on \
                 again: a grant made for an earlier build does not carry \
                 over. Then \u{2318}R.",
                widgets::human_count(denied)
            ))
            .children(refused_paths(app, theme))
            .child(
                div()
                    .flex()
                    .flex_row()
                    ...
                    .child(
                        ...
                        .on_click(|_, _, cx| cx.open_url(FULL_DISK_ACCESS_URL)),
                    )
                    .child(
                        button(
                        ...
                        .on_click(cx.listener(
                            |this, _, _, cx| {
                                this.dismiss_privacy_notice(cx);
                            },
                        )),
                    ),
            ),
...
/// Rows the privacy notice shows before it says how many more there are.
const REFUSED_ROWS: usize = 4;

/// The first few paths the privacy protection refused, so the notice can
/// be checked against what Full Disk Access is expected to unlock: a
/// grant that is not taking effect shows `~/Library/Mail` and its like
/// here, while a path that stays refused with the grant in place is a
/// different problem.
fn refused_paths(app: &Disktree, theme: &Theme) -> Option<Div> {
    let refused: Vec<String> = app
        .progress
        .messages
        .iter()
        .filter_map(|message| {
            message
                .strip_suffix(": not permitted (privacy)")
                .map(|path| {
    ...
    let more = refused.len().saturating_sub(REFUSED_ROWS);
    let mut list = div()
        .debug_selector(|| "refused-paths".into())
        .flex()
        .flex_col()
        .gap(space::XXS)
        .text_size(text::CAPTION)
        .text_color(theme.secondary);
    for path in refused.into_iter().take(REFUSED_ROWS) {
        list = list.child(
            div()
                .whitespace_nowrap()
                .overflow_hidden()
    ...
    if more > 0 {
        list = list.child(format!("and {more} more of the first 50 errors"));
    }
    Some(list)
}
...
fn disk_section(
    ...
    section
        ...
        // Finder counts purgeable space as free; `df` and the meter above
        // do not, because it only comes back when macOS decides to give
        // it. Shown as its own line so the two figures can be reconciled.
        .when(space_info.purgeable > 0, |this| {
            this.child(
                div()
                    .debug_selector(|| "purgeable".into())
                    .text_size(text::CAPTION)
                    .text_color(theme.secondary)
                    .child(format!(
                        "{} purgeable: freed by macOS on demand",
                        human_bytes(space_info.purgeable)
                    )),
            )
        })
        .children(
...
fn find_field(app: &Disktree, theme: &Theme) -> Div {
    ...
    div()
        ...
        .text_size(text::BODY)
        .child(icon(IconName::Search).size(icon::SM).text_color(
            if app.filter_applied {
                highlight
            } else {
                theme.secondary
...
fn review_summary(
    ...
    let mode = button_group(
        "removal-mode",
        vec![
            ChoiceItem::new("trash", RemovalMode::Trash.label())
                .disabled(!trash),
            ChoiceItem::new("permanent", RemovalMode::Permanent.label()),
        ],
    ...
    let explanation = match app.removal_mode {
        RemovalMode::Trash => format!(
            "Recoverable from {} until it is emptied.",
    ...
    for (path, outcome) in app.run_log.iter().rev().take(200) {
        ...
        log = log.child(
            div()
                ...
                .text_size(text::CAPTION)
                .child(
                    icon(if ok { IconName::Check } else { IconName::X })
                        .size(icon::SM)
                        .text_color(if ok {
                            theme.success
                        } else {
...
fn done(
    ...
    div()
        ...
        .flex_1()
        .min_h_0()
        .child(screen_header("Done", "removal finished", &theme, window))
        .child(body)
        .child(
            div()
                .flex()
...
pub fn cursor_tooltip(
    ...
    // The same surface treatment as the controls' tooltip, so an anchored
    // surface of this app does not drift from the system's. Translucent so
    // the mosaic stays visible underneath it.
    let theme = cx.theme();
    Some(
        div()
            .absolute()
...
fn help_overlay(app: &Disktree, cx: &gpui_kit::App) -> Div {
    ...
    let rows: &[(&str, &str)] = &[
        ...
        (
            "trackpad",
            "Two fingers pan \u{00b7} \u{2318} two fingers zoom",
        ),
        ("[ / ]", "Draw fewer or more levels at once"),
        ("- / = / 0", "Magnify, shrink, or reset the view"),
        ("\u{2318} = / - / 0", "Interface zoom"),
        (
            "/ or \u{2318} F",
        ...
        ("r or \u{2318} R", "Scan again from the same root"),
        ("g", "The whole disk; click any directory above to widen"),
        ("", "The first crumb's \u{25be} lists the other volumes"),
        ("\u{2318} O", "Open a folder\u{2026}"),
        ("d", "Disk usage or apparent size"),
        ("i", "Include or skip hidden entries"),
        ("p", "Show or hide the selection line"),
        ("\u{2318} W", "Close the window"),
        ("q or \u{2318} Q", "Quit"),
        ("", ""),
        (
            "Review screen",
