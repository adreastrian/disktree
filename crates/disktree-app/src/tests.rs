...
use gpui_kit::{
    App, Bounds, Context, Entity, Modifiers, Pixels, Point, ScrollDelta,
    ScrollWheelEvent, SharedString, TestAppContext, TouchPhase,
...
/// What `main` sets up before the window: the theme, and the keymap the
/// ⌘ chords live in, so a test pressing `cmd-=` takes the app's own path.
fn init(cx: &mut App) {
    crate::theme::init(cx);
...
/// The top row is drawn in the title bar. At the smallest window `main`
/// allows, the band is the top of the window and as tall as its token,
/// the traffic lights are centred on it, everything in it starts past
/// them and ends inside the window, and the trail never runs into the
/// controls.
#[gpui_kit::test]
fn the_title_bar_clears_the_traffic_lights_at_the_minimum_size(
    cx: &mut TestAppContext,
) {
    use crate::ui::{chrome, size};
    ...
    let bar = cx.debug_bounds("title-bar").expect("the title bar");
    let logo = cx.debug_bounds("logo").expect("the logo");
    let trail = cx.debug_bounds("trail").expect("the trail");
    let settings = cx.debug_bounds("view-settings").expect("the controls");
    let (width, rem) = cx
        .update(|window, _| (window.viewport_size().width, window.rem_size()));
    assert_eq!(width, px(900.));
    assert_eq!(bar.origin, Point::default(), "the band is the window's top");
    assert_eq!(bar.size.width, width);
    assert_eq!(bar.size.height, size::TITLE_BAR.to_pixels(rem));

    // The lights sit on the bar's centre line, in the bar.
    let lights = chrome::traffic_light_position(rem.as_f32());
    assert!(lights.y > px(0.) && lights.y < bar.size.height);
    let centre = lights.y + px(6.);
    assert!(
        (centre - bar.size.height * 0.5).abs() <= px(0.5),
        "lights at {lights:?} in a {:?} bar",
        bar.size.height
    );

    for (name, bounds) in
        [("logo", logo), ("trail", trail), ("controls", settings)]
    {
        assert!(
            bounds.origin.x >= chrome::TRAFFIC_LIGHTS,
            "{name} starts at {:?}, under the lights",
            bounds.origin.x
        );
        assert!(
            bounds.origin.y >= bar.origin.y && bounds.bottom() <= bar.bottom(),
            "{name} at {bounds:?} leaves the {bar:?} band"
        );
    }
    assert!(logo.right() <= trail.origin.x, "{logo:?} then {trail:?}");
    assert!(
        trail.right() <= settings.origin.x,
        "{trail:?} runs into {settings:?}"
    );
    assert!(settings.right() <= width, "{settings:?} leaves the window");
}

/// The window's appearance picks the palette: both dark variants, vibrant
...
fn the_appearance_picks_the_palette() {
    ...
    for (appearance, expected) in [
        (WindowAppearance::Light, ThemeAppearance::Light),
        (WindowAppearance::VibrantLight, ThemeAppearance::Light),
        (WindowAppearance::Dark, ThemeAppearance::Dark),
...
/// Text set in the theme's font shapes, and the base tokens carry the same
/// face, so nothing in the window falls back to a different family.
#[gpui_kit::test]
fn the_theme_font_shapes_text(cx: &mut TestAppContext) {
    cx.update(init);
    let temp = fixture();
    let (_, cx) = view_over(temp.path(), cx);
    cx.update(|window, cx| {
        ...
        assert_eq!(base.tokens.typography.sans, theme.font);
        assert_eq!(base.tokens.typography.mono, theme.mono);
        let style = gpui_kit::TextStyle {
            font_family: theme.font,
            ..window.text_style()
        };
        let text: SharedString = "disktree 1.2 GB".into();
        ...
        assert!(line.width > gpui_kit::px(0.));
    });
}
...
/// `⌘ =` / `⌘ -` / `⌘ 0` change the window's rem, which every size
...
fn interface_zoom_scales_the_rem_and_the_header_band(cx: &mut TestAppContext) {
    ...
    press(cx, "cmd-0");
    assert!((rem(cx) - base).abs() < 0.01, "⌘ 0 resets");

    // The ctrl spellings are kept for a keyboard without ⌘.
    press(cx, "ctrl-=");
    assert!(rem(cx) > base, "ctrl = still zooms in");
    press(cx, "ctrl-0");
    assert!((rem(cx) - base).abs() < 0.01, "ctrl 0 still resets");
}

/// A ⌘ chord is a shortcut or nothing: unbound, it neither types into the
/// find field nor fires the letter under it; bound, it does its own thing
/// even while the field is open.
#[gpui_kit::test]
fn command_chords_never_reach_the_letter_keys(cx: &mut TestAppContext) {
    cx.update(init);
    let temp = fixture();
    let (view, cx) = view_over(temp.path(), cx);
    draw(cx);

    // `x` marks; `⌘ x` is bound to nothing and must not.
    press(cx, "cmd-x");
    assert!(read(&view, cx, |app| app.marks.is_empty()));
    press(cx, "x");
    assert!(!read(&view, cx, |app| app.marks.is_empty()));
    ...
    press(cx, "cmd-f");
    assert!(read(&view, cx, |app| app.find_open), "⌘ F opens the field");
    press(cx, "cmd-x");
    assert_eq!(read(&view, cx, |app| app.find.clone()), "");
    assert!(read(&view, cx, |app| app.find_open), "and stays open");
    press(cx, "j");
    assert_eq!(read(&view, cx, |app| app.find.clone()), "j");
    // ⌘ R is bound: it rescans, even with the field open, and the field
    // goes with the tree it was searching.
    press(cx, "cmd-r");
    assert!(read(&view, cx, |app| app.tree().is_none()));
    assert!(!read(&view, cx, |app| app.find_open));
}

/// The menu's actions are the plain keys' methods: each one moves the same
/// state the key does.
#[gpui_kit::test]
fn menu_actions_do_what_the_keys_do(cx: &mut TestAppContext) {
    cx.update(init);
    let temp = fixture();
    let (view, cx) = view_over(temp.path(), cx);
    draw(cx);

    cx.dispatch_action(crate::menu::ShowHelp);
    assert!(read(&view, cx, |app| app.show_help));
    cx.dispatch_action(crate::menu::ShowHelp);
    assert!(!read(&view, cx, |app| app.show_help));

    cx.dispatch_action(crate::menu::ToggleReview);
    assert_eq!(read(&view, cx, |app| app.screen), Screen::Explore);
    assert!(
        read(&view, cx, |app| app.notice.is_some()),
        "nothing marked"
    );
    press(cx, "x");
    cx.dispatch_action(crate::menu::ToggleReview);
    ...
    cx.dispatch_action(crate::menu::Rescan);
    assert!(read(&view, cx, |app| app.scan.is_some()));
}

/// The Finder's Open With and a folder dropped on the Dock replace the
/// root: the new tree is scanned from there, and the old one is gone.
#[gpui_kit::test]
fn opening_a_folder_replaces_the_root_and_rescans(cx: &mut TestAppContext) {
    cx.update(init);
    let temp = fixture();
    let (view, cx) = view_over(temp.path(), cx);
    draw(cx);
    ...
    update(&view, cx, |app, cx| app.open_folder(junk.clone(), cx));
    draw(cx);
    let canonical = junk.canonicalize().expect("canonical");
    assert_eq!(read(&view, cx, |app| app.root_path.clone()), canonical);
    assert!(read(&view, cx, |app| app.tree().is_none()));
    ...
    assert!(names.iter().any(|name| name == "blob.bin"), "{names:?}");
    assert!(names.iter().any(|name| name == "deeper"), "{names:?}");

    // A file is not a root: nothing changes.
    let file = canonical.join("blob.bin");
    update(&view, cx, |app, cx| app.open_folder(file, cx));
    assert_eq!(read(&view, cx, |app| app.root_path.clone()), canonical);
}

#[test]
fn a_file_url_names_a_folder_with_its_escapes_undone() {
    let temp = tempfile::TempDir::new().expect("tempdir");
    let folder = temp.path().join("My Folder");
    std::fs::create_dir(&folder).expect("mkdir");
    let url = format!(
        "file://{}/My%20Folder/",
        temp.path().to_str().expect("utf-8 tempdir")
    );
    assert_eq!(crate::folder_from_url(&url), Some(folder.clone()));
    let with_host = format!(
        "file://localhost{}/My%20Folder",
        temp.path().to_str().expect("utf-8 tempdir")
    );
    assert_eq!(crate::folder_from_url(&with_host), Some(folder));

    std::fs::write(temp.path().join("file.txt"), b"x").expect("write");
    let file = format!(
        "file://{}/file.txt",
        temp.path().to_str().expect("utf-8 tempdir")
    );
    assert_eq!(crate::folder_from_url(&file), None);
    assert_eq!(crate::folder_from_url("https://example.com/"), None);
    assert_eq!(crate::folder_from_url("file:///no/such/%zz"), None);
}

/// A scan the OS's privacy protection refused somewhere says so in the
/// panel, with the way to System Settings, until it is dismissed.
#[gpui_kit::test]
fn a_refused_scan_shows_the_full_disk_access_notice(cx: &mut TestAppContext) {
    cx.update(init);
    let temp = fixture();
    let (view, cx) = view_over(temp.path(), cx);
    draw(cx);
    assert!(cx.debug_bounds("privacy-notice").is_none());

    update(&view, cx, |app, cx| {
        app.progress.denied = 3;
        app.progress.errors = 3;
        app.progress.messages = vec![
            "/Users/x/Library/Mail: not permitted (privacy)".into(),
            "/Users/x/.Trash: not permitted (privacy)".into(),
            "/Users/x/Library/Safari: not permitted (privacy)".into(),
        ];
        cx.notify();
    });
    draw(cx);
    assert!(
        cx.debug_bounds("privacy-notice").is_some(),
        "notice painted"
    );
    // It names what was refused, so a grant that is not taking effect can
    // be told from a path the grant does not cover.
    assert!(cx.debug_bounds("refused-paths").is_some(), "paths listed");

    update(&view, cx, Disktree::dismiss_privacy_notice);
    draw(cx);
    assert!(cx.debug_bounds("privacy-notice").is_none(), "dismissed");
    assert!(read(&view, cx, |app| app.privacy_denials().is_none()));
}

/// Purgeable space is its own line, and never part of the meter's numbers.
#[gpui_kit::test]
fn purgeable_space_is_shown_beside_the_meter(cx: &mut TestAppContext) {
    cx.update(init);
    let temp = fixture();
    let (view, cx) = view_over(temp.path(), cx);
    let space = SpaceInfo {
        ...
        available: 180_000_000_000,
        purgeable: 0,
    };
    update(&view, cx, |app, cx| {
        app.space = Some(space);
        cx.notify();
    });
    draw(cx);
    assert!(cx.debug_bounds("purgeable").is_none(), "nothing to purge");

    update(&view, cx, |app, cx| {
        app.space = Some(SpaceInfo {
            purgeable: 12_100_000_000,
            ..space
        });
        cx.notify();
    });
    draw(cx);
    assert!(cx.debug_bounds("purgeable").is_some(), "purgeable line");
    let after = read(&view, cx, |app| {
        app.space.expect("space").after_removing(1_000).available
    });
    assert_eq!(after, space.available + 1_000, "projection ignores it");
}

/// A trackpad's two-finger scroll pans the magnified view; with ⌘ held it
/// zooms; a mouse wheel's notches zoom as they always did.
#[gpui_kit::test]
fn trackpad_scrolls_pan_and_command_scrolls_zoom(cx: &mut TestAppContext) {
    cx.update(init);
    let temp = fixture();
    let (view, cx) = view_over(temp.path(), cx);
    draw(cx);
    ...
    let scroll = |delta: ScrollDelta, modifiers: Modifiers| ScrollWheelEvent {
        position: middle,
        delta,
        modifiers,
    ...
    let pixels = |x: f32, y: f32| ScrollDelta::Pixels(Point::new(px(x), px(y)));

    // Unmagnified, there is nowhere to pan.
    update(&view, cx, |app, cx| {
        app.on_scroll_wheel(
            &scroll(pixels(0.0, -30.0), Modifiers::default()),
            cx,
        );
    });
    let (scale, origin_y) =
        read(&view, cx, |app| (app.view.scale, app.view.origin_y));
    assert!((scale - 1.0).abs() < f32::EPSILON, "unmagnified: {scale}");
    assert!(origin_y.abs() < f32::EPSILON, "unpanned: {origin_y}");

    // ⌘ and two fingers: zoom, smoothly rather than a notch per event.
    update(&view, cx, |app, cx| {
        app.on_scroll_wheel(&scroll(pixels(0.0, 20.0), command), cx);
    });
    let scale = read(&view, cx, |app| app.view.scale);
    assert!(scale > 1.0 && scale < 1.15, "smooth zoom: {scale}");

    // Magnify well past that, then two fingers alone pan.
    update(&view, cx, |app, cx| {
        for _ in 0..8 {
            app.on_scroll_wheel(&scroll(pixels(0.0, 40.0), command), cx);
        }
    });
    let (scale, origin_y) =
        read(&view, cx, |app| (app.view.scale, app.view.origin_y));
    assert!(scale > 1.5, "magnified: {scale}");
    update(&view, cx, |app, cx| {
        app.on_scroll_wheel(
            &scroll(pixels(0.0, -30.0), Modifiers::default()),
            cx,
    ...
    let panned = read(&view, cx, |app| app.view.origin_y);
    assert!(panned > origin_y, "panned down: {origin_y} -> {panned}");
    let still = read(&view, cx, |app| app.view.scale);
    assert!(
        (still - scale).abs() < f32::EPSILON,
        "no zoom: {scale} {still}"
    );

    // A wheel notch is the old behaviour: one step out.
    update(&view, cx, |app, cx| {
        app.on_scroll_wheel(
            &scroll(
    ...
    let out = read(&view, cx, |app| app.view.scale);
    assert!(
        (out - scale / 1.15).abs() < 0.01,
        "one notch out: {scale} -> {out}"
    );
}
...
fn widening_reuses_the_tree_it_has_and_reads_only_the_rest(
    ...
    // The trail runs from the top of the volume, and the scanned root sits
    // under it.
...
/// The trail begins at the volume the root lives on, and the first
/// crumb's ▾ lists the Mac's volumes: the way to another disk, since a
/// scan never crosses into one and the trail cannot climb to it.
#[gpui_kit::test]
fn the_volume_menu_lists_the_disks_and_the_trail_starts_at_one(
    cx: &mut TestAppContext,
) {
    cx.update(init);
    let temp = fixture();
    ...
    // A temporary directory is on the boot disk, whose top is "/", so the
    // trail runs from "/" and never above it.
    let (disk, first) = read(&view, cx, |app| {
        (app.disk_root.clone(), app.breadcrumbs()[0].0.clone())
    });
    assert_eq!(disk.as_deref(), Some(Path::new("/")));
    assert_eq!(first, "/");

    let chevron = cx
        .debug_bounds("volume-menu-chevron")
        .expect("the volume crumb has a menu");
    cx.simulate_click(chevron.center(), Modifiers::none());
    draw(cx);
    assert!(
        cx.debug_bounds("volume-menu").is_some(),
        "the menu is drawn"
    );
    let (volumes, highlighted) = read(&view, cx, |app| {
        let menu = app.volume_menu.clone().expect("open");
        (app.volumes.clone(), menu.highlighted)
    });
    // The boot disk is first, under its Finder name, and it is the one
    // highlighted because it is the one on screen.
    assert!(volumes[0].is_boot, "{volumes:?}");
    assert_eq!(volumes[0].root, Path::new("/"));
    assert_eq!(highlighted, 0);

    // Escape closes it, and the scan is untouched.
    press(cx, "escape");
    assert!(read(&view, cx, |app| app.volume_menu.is_none()));
    assert!(cx.debug_bounds("volume-menu").is_none());
    assert_eq!(read(&view, cx, |app| app.root_path.clone()), temp.path());
}
