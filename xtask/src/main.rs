...
//! aggregates (`cargo xtask lint`, `cargo xtask ci`), mirroring how omatrack
//! exposes every linter both as a build target and as a labelled test. These
//! run locally and nowhere else; `lint` never fixes anything: it reports the
//! diff and fails, so a red run is the signal, not a repair.
